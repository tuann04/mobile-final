...
class NotificationService {
  final FlutterLocalNotificationsPlugin sleepLocalNotificationsPlugin  = FlutterLocalNotificationsPlugin();
  ...
  Future<void> initNotification() async {
    ...
    await sleepLocalNotificationsPlugin.initialize(
      initializationSettings,
      onDidReceiveNotificationResponse: (NotificationResponse notificationResponse) async {
        if (true) {
          await scheduleNextDaySleepNotification(notificationResponse);
        }
      },
    );
  ...
  Future<void> _requestPermissions() async {
    // iOS permissions
    await sleepLocalNotificationsPlugin
  ...
  Future<void> showNotification({int id = 0, String? title, String? body, String? payload}) async {
    return sleepLocalNotificationsPlugin.show(
      id, title, body,
  ...
  notificationDetails() {
    return const NotificationDetails(
      android: AndroidNotificationDetails(
        'sleep_channel', // ID of the notification channel
        'Sleep Notifications', // Name of the notification channel
  ...
  Future<void> scheduleNotification({
    ...
    return sleepLocalNotificationsPlugin.zonedSchedule(
      id,
      title,
      body,
      tz.TZDateTime.from(
  ...
  Future<void> scheduleNextDaySleepNotification(NotificationResponse response) async {
    ...
    // Toast notification
    Fluttertoast.showToast(
      msg: 'Scheduled sleep reminder for the next day',
      toastLength: Toast.LENGTH_SHORT,
      gravity: ToastGravity.BOTTOM,
      timeInSecForIosWeb: 1,
      backgroundColor: Colors.black,
